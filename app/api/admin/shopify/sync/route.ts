  ...
  try {
    ...
    for (const sp of shopifyProducts) {
      ...
      const existing = await sql`
        SELECT id FROM products WHERE seller_id = ${user.id} AND external_store_id = ${store.id} AND listing_code LIKE ${"SHOPIFY-ADMIN-" + shopifyProductId + "-%"}
      `;
      ...
      if (existing.length > 0) {
        ...
        const listingCode = `SHOPIFY-ADMIN-${shopifyProductId}-${generateListingCode()}`;
