...
export default function AdminProductForm({ onCreated }: { onCreated: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSuccess(`✅ Listed! Code: ${data.product.listing_code}`);
    setName(""); setDescription(""); setBasePrice(""); setMarketPrice(""); setStockQty(""); setImages([]);
    onCreated();
  }

  return (
    <form onSubmit={handleSubmit} className="glass-card rounded-2xl p-6 space-y-4">
      <h2 className="text-xl font-bold text-white mb-2">📦 List a Product (as Admin)</h2>
      <input
        required
      ...
      <div>
        ...
        <input type="file" multiple accept="image/*" onChange={handleImageUpload} className="text-sm text-galaxy-300" />
          ...
          <div className="flex gap-2 mt-3 flex-wrap">
            {images.map((img, i) => (
              <div key={i} className="relative w-16 h-16 rounded-lg overflow-hidden border border-galaxy-400/30">
                ...
                <button type="button" onClick={() => removeImage(i)} className="absolute top-0 right-0 bg-red-600 text-white text-xs w-5 h-5 flex items-center justify-center">×</button>
              </div>
            ))}
          </div>
        )}
